...
function Keyboard:new(textEdit, x, y, config, background, button, selected, click)
	...
	dofile(SB_LUAGUI_PATH .. config ..".conf")
	version = config
	Keyboard._alfanumericoDefault = ALFANUMERICO_DEFAULT
    ...
    if setKeyboard then setKeyboard(instance) end
    return instance    
end
...
function Keyboard:setTextEdit(textEdit)
	if setEdit then setEdit(textEdit) end
...
if openKeyboardOnDivice then openKeyboardOnDivice(bollRunning) end
